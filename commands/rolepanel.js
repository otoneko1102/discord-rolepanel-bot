  ...
  async execute(client, message, args, config) {
    ...
    try {
      ...
      if (!replied) {
        ...
        const messageId = selectedRolePanel[`${message.author.id}_${message.guild.id}`]?.message_id || null;
        const channelId = selectedRolePanel[`${message.author.id}_${message.guild.id}`]?.channel_id || null;
        if (messageId && channelId) {
          replied = { messageId, channelId };
        }
      }
      ...
      if (mode === 'create') {
        ...
        await message.channel.send(`${config.check} 成功しました`)
          .then(msg => {
          setTimeout(async () => {
            await msg.delete();
          }, 3000)
      ...
      let repliedMessage;
      try {
        repliedMessage = await message.guild.channels.cache.get(replied.channelId).messages.fetch(replied.messageId);
      } catch {
        err_embed.setDescription(`❌ ロールパネルを選択してください`).setColor('RED');
        await message.channel.send({ embeds: [err_embed] });
        return;
